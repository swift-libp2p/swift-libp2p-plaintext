...
class IntegrationTests: XCTestCase {
    ...
    func testInternalInterop() throws {
        ...
        client.newRequest(to: host.listenAddresses.first!, forProtocol: "/echo/1.0.0", withRequest: "Hello Swift LibP2P".data(using: .utf8)!, withHandlers: .handlers([.newLineDelimited])).whenComplete { res in
            switch res {
    ...
    func testGoHostInterop() throws {
        ...
        let response = try client.newRequest(to: Multiaddr("/ip4/127.0.0.1/tcp/10000"), forProtocol: "/echo/1.0.0", withRequest: "Hello Swift LibP2P".data(using: .utf8)!, withHandlers: .handlers([.newLineDelimited])).wait()
    ...
    func testJSInterop() throws {
        ...
        /// Fire off an echo request to the go echo server on port 10000
        let response = try client.newRequest(to: Multiaddr("/ip4/127.0.0.1/tcp/10333"), forProtocol: "/echo/1.0.0", withRequest: "Hello Swift LibP2P".data(using: .utf8)!, withHandlers: .handlers([.newLineDelimited])).wait()
    ...
    private func makeLocalEchoHost(port:Int) throws -> Application {
        ...
        lib.routes.group("echo", handlers: [.newLineDelimited]) { echo in
            echo.on("1.0.0") { req -> Response<ByteBuffer> in
                ...
                case .error(let error): req.logger.error("\(error)"); return .close
                }
            }
        }
